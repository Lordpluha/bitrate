...
- Дизайн приложения делаем в `pencil/web-artist-design/web-artist.pen`.
...
- [x] Сохранить три темы: Dark, Light, Dim. Основной нейтральный вариант согласован
  как Dim; не создавать отдельную новую палитру вместо уже существующих токенов.
...
Первая итерация раздела 2 подготовлена в `web-artist.pen`:
...
Dim зафиксирован владельцем как основной нейтральный вариант. Dark остаётся
контрастной альтернативой, Light — светлой; все три режима сохранены в переменных.
...
Это локальные изображения для проверки, исходник — `web-artist.pen`.
...
по-прежнему находятся в `web-artist.pen`.
