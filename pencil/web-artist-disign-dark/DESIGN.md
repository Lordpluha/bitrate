...
**Creative North Star: "Dark Signal"**

Глубокий почти чёрный кабинет с холодными поверхностями и электрическим violet.
Контраст и свечение направляют к следующему действию, а не превращают каждый
элемент в неоновую декорацию. Музыкальную индивидуальность по-прежнему несут обложки.
...
Источник истины этой ветки — `web-artist-dark.pen`, включая переменные и мастера.
Это отдельная контрастная Dark-разведка; согласованный нейтральный Dim в исходной
ветке не перезаписывается. При расхождении сверяться с dark-файлом через Pencil.

**Key Characteristics:**

- Почти чёрное основание, холодные многоуровневые поверхности и спектральные акценты.
...
Срез: 10 сентября 2026. Проверены статические снимки системных разделов `00–08`:
...
Electric violet выделяет действия и выбор на почти чёрном фоне; cyan и pink
используются как редкие спектральные акценты, а не как конкурирующие CTA.
...
- **Don't** Переносить контрастный Dark-поднабор в нейтральный Dim без отдельной проверки theme tokens и читаемости.
