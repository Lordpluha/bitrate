# react: detailed guidance

Read only the section needed for this task. The scoped rule
[../rules/react.md](../rules/react.md) owns the concise requirements.
...
More than two effects prompts a review of external synchronization and coupled state.
Keep cohesive code when appropriate; explain the decision in review, not source comments.
...
Follow `.claude/rules/code-principles.md`: size and props thresholds are review signals.
Explain retain/split in review; do not split solely to satisfy a number.
