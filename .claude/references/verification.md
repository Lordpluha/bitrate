...
Use `br-verify` as the workflow entrypoint. Run heavy checks through
`python3 -B .claude/scripts/run-heavy.py -- <command> <args...>`; busy/failed preflight
means defer or narrow scope. The launcher locks before measuring resources. [Hook policy](hook-policy.md) describes formatter routing and limits.
...
For affected Biome workspaces, use the mutating form covering all three:

```bash
pnpm --filter @bitrate/<workspace> exec biome check --write <changed-paths>
```
...
Run the relevant workspace gates when code or configuration changed:
```bash
pnpm --filter @bitrate/<workspace> lint
pnpm --filter @bitrate/<workspace> check-types
```

For documentation-only edits, validate links, structure and whitespace instead. Run broader
checks only when the change affects multiple workspaces or shared tooling.
